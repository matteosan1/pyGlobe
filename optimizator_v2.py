...
for z in xrange(t1.GetEntries()):
   t1.GetEntry(z)
   if (t1.cat == 0 and t1.mass > 100.):
      if (t1.id2 == 11):
         if (t1.id1 > 0.5):
            if (t1.iso1 < 0.5):
               if (t1.met < 70):
                  if (t1.njets20 < 8):
                     # id1, iso1, met, njets20, btag1, btag2
                     if (t1.itype < 0):
                        sigTuple.append([t1.id1, t1.iso1, t1.met, t1.njets20, t1.btag1, t1.btag2, t1.weight])
                     else:
                        bkgTuple.append([t1.id1, t1.iso1, t1.met, t1.njets20, t1.btag1, t1.btag2, t1.weight])
...
# id1, iso1, met, njets20, btag1, btag2
conditions = [[0.8, 0.0001, 0.5, 1., False], [0.2, 0.0001, 0.05, 0.5, True], [50, 0.01, 10, 70., True], [5, 0.1, 0, 8, True], [.9, 0.001, 0.3, 1., True]]
